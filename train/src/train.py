...
def main():
    ...
    if training_args.use_int8_training:
        ...
        if "chatglm2" not in model_args.model_name_or_path:
            model = AutoModelForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                torch_dtype=torch_dtype,
            )
    ...
    def generate_and_tokenize_prompt(data_point):
        ...
        for sentence in source:
            ...
            sentence_value = sentence["value"] if sentence_from == 'human' else \
            ...
            # inputs = tokenizer(sentence_value, add_special_tokens=False)#do not add bos_token_id
            # sentence_ids = inputs['input_ids']
            ...
            if sentence_from != 'human':
                if 'chatglm2' not in model_args.model_name_or_path:
                    ...
                    input_ids += [tokenizer.get_command('<eos>')]  # make sur+e eos_token_id is correct
                    labels += [tokenizer.get_command('<eos>')]

        if len(input_ids) > training_args.model_max_length:
            # print("exceed!!", len(input_ids))
            sum += 1
            # left truncate
            input_ids = input_ids[-training_args.model_max_length:]
            labels = labels[-training_args.model_max_length:]
    ...
    trainer = Trainer(
        ...
        data_collator=transformers.DataCollatorForSeq2Seq(tokenizer, pad_to_multiple_of=8, return_tensors="pt",
                                                          padding=True),
        callbacks=[SavePeftModelAtEndCallback()],
    )
    ...
    # import pdb; pdb.set_trace()
    peft_model_path = os.path.join(training_args.output_dir, "adapter_model")
