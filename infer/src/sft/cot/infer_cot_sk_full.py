...
PREDICT_SQL_SK_FULL_PROMPT = """### SQLite SQL tables are requested to be represented in the following format.
TABLE_NAME (
COLUMN_NAME: TYPE, (DESCRIPTION), (VALUE1, VALUE2, ...) 
)
FOREIGN KEYS:
...
if __name__ == '__main__':
    ...
    for i, dev_data in enumerate(dev_datas):
        ...
        predict_sql_input_str = PREDICT_SQL_SK_FULL_PROMPT.format(table_info=predict_sql_schema_A, note=note, question=question,
            ...
            if len(can_delete_tables) > 0:
                ...
                for column in this_can_delete_columns_list:
                    if is_primary_or_foreign_key(table_info, delete_columns_of_table, column):
                        ...
                        columns_of_tables[delete_columns_of_table].remove(column)
                        break
            schema_A = generate_deleted_schema_A(schema_A_info[db_name], skip_tables=delete_tables, skip_columns=delete_columns)
            input_str = PREDICT_SQL_SK_FULL_PROMPT.format(table_info=schema_A, note=note, question=question, sql_skeleton = predict_sql_skeleton_output,
