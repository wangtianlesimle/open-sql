...
if __name__ == '__main__':
    ...
    schema_D_info_file = "/public13_data/wtl/work_point/open_sql_v1/datasets/bird/dev/schema/schema_D.json"
    schema_D_info = json.load(open(schema_D_info_file, "r"))
    schema_A_info_file = "/public13_data/wtl/work_point/open_sql_v1/datasets/bird/dev/schema/schema_A.json"
    schema_A_info = json.load(open(schema_A_info_file, "r"))
    table_file = "/public13_data/wtl/work_point/open_sql_v1/datasets/bird/dev/schema/table_info.json"
    table_data = json.load(open(table_file, "r"))
    dev_tables_info_file = "/public13_data/wtl/work_point/open_sql_v1/datasets/bird/dev/dev_tables.json"
            ...
            if len(can_delete_tables) > 0:
                ...
                for column in this_can_delete_columns_list:
                    if is_primary_or_foreign_key(table_info, delete_columns_of_table, column):
                        ...
                        columns_of_tables[delete_columns_of_table].remove(column)
                        break
            predict_sql_schema_A = generate_deleted_schema_A(schema_A_info[db_name], skip_tables=delete_tables, skip_columns=delete_columns)
            predict_sql_input_str = PREDICT_SQL_PROMPT.format(table_info=predict_sql_schema_A, note=note, question=question,
