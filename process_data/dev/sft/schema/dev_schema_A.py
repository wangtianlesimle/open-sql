...
def get_dev_schema_A():
    ...
    f = open("/public14_data/wtl/work_point/open_sql_v1/datasets/bird/dev/sft/schema/dev_schema_A.json", "w")
    dev_schema_A = {}
    for id, data in enumerate(dev_data):
        db_name = data["db_id"]
        dev_schema_A["id"] = id
        dev_schema_A["db_id"] = db_name
        question = data["question"]
        note = data["evidence"]
        sql = data["SQL"]
        schema = SCHEMA_A.format(table_info=generate_schema_A(schema_info[db_name]), question=question, note=note)
        dev_schema_A["conversations"] = [{"from": "human", "value": schema},
                                                 {"from": "assistant", "value": sql}]
        f.write(json.dumps(dev_schema_A, ensure_ascii=False) + "\n")
