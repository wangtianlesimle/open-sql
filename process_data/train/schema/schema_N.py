...
def generate_schema_N():
    ...
    schema_file = "/public14_data/wtl/work_point/open_sql_v1/datasets/bird/train/schema/schema_N.json"
    schema_F = {}
    foreign_keys = get_foreign_keys()
    for idx, database_name in enumerate(database_names):
        # print(f"database_name:{idx}")
        schema_F[database_name] = {}
        # 获取数据库对应的文件路径
        ...
        for t_idx, table in enumerate(tables):
            ...
            if table_name == "sqlite_sequence":
                continue
            schema_F[database_name][table_name] = []
            cursor.execute(f"PRAGMA table_info('{table_name}')")
            # 获取查询结果
            columns = cursor.fetchall()
            for c_idx, column in enumerate(columns):
                column_name = column[1]
                schema_F[database_name][table_name].append(column_name)

        schema_F[database_name]["foreign_keys"] = foreign_infos
        # 关闭数据库连接
        conn.close()
    with open(schema_file, 'w') as json_file:
        json.dump(schema_F, json_file, indent=4, ensure_ascii=False)
