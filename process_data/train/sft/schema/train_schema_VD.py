...
SCHEMA_VD = """### Complete sqlite SQL query only and with no explanation
...
def get_train_schema_VD():
    schema_info_file = "/public14_data/wtl/work_point/open_sql_v1/datasets/bird/train/schema/schema_VD.json"
    ...
    f = open("/public14_data/wtl/work_point/open_sql_v1/datasets/bird/train/sft/schema/train_schema_VD.json", "w")
    train_schema_VD = {}
    for id, data in enumerate(train_data):
        ...
        note = data["evidence"]
        sql = data["SQL"]
        schema = SCHEMA_VD.format(table_info=generate_schema_VD(schema_info[db_name]), question=question, note=note)
