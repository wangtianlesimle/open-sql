...
SCHEMA_V = """### Complete sqlite SQL query only and with no explanation
### SQLite SQL tables are requested to be represented in the following format.
TABLE1_NAME (
COLUMN_NAME: (ENUM_VALUE, ENUM_VALUE2, ...)
)
FOREIGN KEYS:
...
def get_train_schema_V():
    schema_info_file = "/public14_data/wtl/work_point/open_sql_v1/datasets/bird/train/schema/schema_V.json"
    ...
    f = open("/public14_data/wtl/work_point/open_sql_v1/datasets/bird/train/sft/schema/train_schema_V.json", "w")
    train_schema_F = {}
    for id, data in enumerate(train_data):
        ...
        note = data["evidence"]
        sql = data["SQL"]
        schema = SCHEMA_V.format(table_info=generate_schema_V(schema_info[db_name]), question=question, note=note)
