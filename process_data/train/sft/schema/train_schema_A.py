...
SCHEMA_A = """### Complete sqlite SQL query only and with no explanation
...
def get_train_schema_A():
    ...
    f = open("/public14_data/wtl/work_point/open_sql_v1/datasets/bird/train/sft/schema/train_schema_A.json", "w")
    train_schema_A = {}
    for id, data in enumerate(train_data):
        db_name = data["db_id"]
        train_schema_A["id"] = id
        train_schema_A["db_id"] = db_name
        question = data["question"]
        note = data["evidence"]
        sql = data["SQL"]
        schema = SCHEMA_A.format(table_info=generate_schema_A(schema_info[db_name]), question=question, note=note)
        train_schema_A["conversations"] = [{"from": "human", "value": schema},
                                                 {"from": "assistant", "value": sql}]
        f.write(json.dumps(train_schema_A, ensure_ascii=False) + "\n")
